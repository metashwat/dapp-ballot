...
contract Ballot {
    ...
    //modifer
    modifier onlyOwner () {
      require(msg.sender == chairperson, "Sender not authorised.");
      _;
    }
    ...
    function register(address toVoter) public onlyOwner{
        if(voters[toVoter].weight != 0) revert("Unknown error has occured.");
    ...
    function vote(uint8 toProposal) public {
        ...
        if (sender.voted || toProposal >= 4 || sender.weight == 0) revert("Unknown error has occured.");
        sender.voted = true;
